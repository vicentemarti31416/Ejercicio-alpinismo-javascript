...
miCamino.material.forEach((objeto) => console.log(`El material que debo llevar es ${objeto.nombre}`))
...
miCamino.material.forEach((objeto, index) => {
  if(index == 0) {
    console.log(`Lo primero que debo llevar es ${objeto.nombre} y es ${objeto.descripcion}`);
  } else if(index == 1) {
    console.log(`Lo segundo que debo llevar es ${objeto.nombre} y es ${objeto.descripcion}`);
  } else if(index == 2) {
    console.log(`Lo tercero que debo llevar es ${objeto.nombre} y es ${objeto.descripcion}`);
  } else {
    console.log(`Por último debo llevar la ${objeto.nombre} que es ${objeto.descripcion}`);
  }
})

//5º: vamos a decir: para subir al {nombre de la cima}  debo subir {altura } metros y debo viajar a {ubicacion}

miCamino.cimas.forEach((cima) => console.log(`Para subir al ${cima.nombre} debo subir ${cima.altitud} metros y debo viajar a ${cima.localizacion}`))
...
miCamino.subiendoAlEverest.forEach((i) => {
  i.forEach((j) => {
...
miCamino.subiendoAlEverest.forEach((i) => {
  i.forEach((j) => {
    j.campoBase.forEach((campo) => {
      campo.campoI.campoII.campoIII.campoIV.forEach((objeto) => {
        if(typeof objeto.cima === "string") console.log(objeto.cima)
      } );
    })
  })
})
...
let direccion = '';
miCamino.material.forEach((item) => {
    item.imagenes.forEach((image) => {
      for(let key in image) {
        for(let url in image[key]) {
          if(typeof image[key][url] === "string") direccion = image[key][url];
        }
      }
      console.log(`La url del item ${item.nombre} es "${direccion}"`)
    })
})
...
miCamino.cimas.forEach((cima) => {
  let altitudString = cima.altitud.toString().replace('.', ''); '8654'
...
console.log(`La altura media de todas las montañas es -> ${Math.round(alturaTotal / contador)} metros`)
